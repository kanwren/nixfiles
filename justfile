...
hecate command="build": hecate-system
    {{ if command == "build" { "" } else { "sudo " + quote(`nix --experimental-features 'nix-command flakes' build --no-link --print-out-paths '.#nixosConfigurations.hecate.config.system.build.toplevel'` / "bin" / "switch-to-configuration") + " " + quote(command) } }}
